  ...
  <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-shadow">
    ...
    <div :class="['px-5 py-4', levelGradient]">
      <div class="flex items-baseline flex-wrap gap-x-3 gap-y-1">
        <router-link :to="`/word/${encodeURIComponent(word.word)}`" class="text-2xl font-bold text-white no-underline hover:underline">
          {{ word.word }}
        </router-link>
        <span class="text-white/70 text-sm">{{ word.ipa }}</span>
        <span class="text-xs bg-white/20 text-white px-2 py-0.5 rounded-full">{{ word.pos }}</span>
        <span class="text-xs font-bold bg-white text-gray-700 px-2 py-0.5 rounded-full ml-auto">{{ word.level }}</span>
      </div>
    ...
    <div class="px-5 py-4 space-y-4">
      ...
      <div v-if="word.definition_it">
        <p class="text-xs text-gray-400 italic leading-relaxed">{{ word.definition_it }}</p>
      </div>
      ...
      <div v-if="word.conjugation">
        <button @click="showConj = !showConj" class="w-full cursor-pointer bg-transparent border-none p-0">
      ...
      <div v-if="word.collocations || word.synonyms || word.antonyms">
        <div v-if="word.collocations && word.collocations.length" class="mb-3">
          ...
          <div class="flex flex-wrap gap-1.5">
            <span v-for="c in word.collocations" :key="c" class="text-xs px-2.5 py-1 rounded-full bg-gray-100 text-gray-600">{{ c }}</span>
          </div>
        </div>
        <div class="flex flex-wrap gap-1.5">
          <span v-for="s in (word.synonyms || [])" :key="'s'+s" class="text-xs px-2.5 py-1 rounded-full bg-emerald-50 text-emerald-700">≈ {{ s }}</span>
          <span v-for="a in (word.antonyms || [])" :key="'a'+a" class="text-xs px-2.5 py-1 rounded-full bg-red-50 text-red-700">⇋ {{ a }}</span>
        </div>
      </div>

      <!-- 派生词族 -->
      <div v-if="word.related && word.related.length">
        <button @click="showRelated = !showRelated" class="w-full cursor-pointer bg-transparent border-none p-0">
        ...
        <div v-if="showRelated" class="space-y-1">
          <div v-for="r in word.related" :key="r.word" class="flex items-baseline gap-2 text-sm">
            <router-link :to="`/word/${encodeURIComponent(r.word)}`" class="font-semibold text-emerald-700 hover:underline no-underline">{{ r.word }}</router-link>
      ...
      <div v-if="word.usage_notes">
        <button @click="showUsage = !showUsage" class="w-full cursor-pointer bg-transparent border-none p-0">
